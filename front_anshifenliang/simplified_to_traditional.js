...
// 🔹 专有词汇映射（优先转换）
const specialTermsMap = {
  '钟表': '鐘錶', '借': '藉', '里': '裏', 
  '预表': '豫表','豫表': '豫表','预备': '豫備', '才干': '才干', '凭着': '憑着', '希奇': '希奇', '彻底': '徹底', '了解': '了解', '扎实': '扎實', '题到': '題到', '豫备': '豫備', '翻译': '繙譯', '普遍': '普徧', '台湾': '臺灣', '台风': '颱风', '天才': '天才', '才能': '才能', '著作': '著作', '重复': '重複', '恢复': '恢復', '复活': '復活', '复习': '復習', '回复': '回覆', '答复': '答覆', '参加': '參加', '参与': '參與', '人参': '人蔘', '西洋参': '西洋蔘', '钻研': '鑽研', '钻洞': '鑽洞', '电钻': '電鑚', '大卫': '大衞', '合并': '合併', '周全': '周全', '周知': '周知', '周详': '周詳', '周遭': '周遭', '周到': '周到', '异象': '異象', '摇荡': '搖蕩', '对象': '對象', '翻作': '繙作', '注解': '註解', '吃': '喫', '神迹': '神蹟', '烧毁': '燒燬', '毁坏': '毁壞', '手表': '手錶', '麻风': '痳瘋', '干才': '干才', '台风': '颱风', "㓦划": "㓦劃",
  "哑吧": "哑吧", "干干净净": "乾乾淨淨",
  "记账": "記賬",
  "帐幕": "帳幕",
  "鉴察": "鑒察",
...
function convertToTraditional(text) {
    ...
    // 1️⃣ 先处理专有词汇
    for (const [simplified, traditional] of Object.entries(specialTermsMap)) {
        processedText = processedText.replace(new RegExp(simplified, "g"), traditional);
    }

    // 2️⃣ 创建一个 Set，记录已转换的专有词范围，避免单字转换干扰
    let convertedPositions = new Set();
    for (const term in specialTermsMap) {
        let index = processedText.indexOf(specialTermsMap[term]);
        while (index !== -1) {
            for (let i = index; i < index + specialTermsMap[term].length; i++) {
                convertedPositions.add(i);  // 记录被转换的索引
            }
            index = processedText.indexOf(specialTermsMap[term], index + 1);
        }
    }

    // 3️⃣ 再处理单字符，但排除已转换的专有词部分
    processedText = processedText.split('').map((char, i) => 
        convertedPositions.has(i) ? char : (simplifiedToTraditionalMap[char] || char)
    ).join('');
