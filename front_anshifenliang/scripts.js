...
function formatMessage(message) {
    ...
    return `<span class="zheng-pian-group"><button class="zheng-pian-btn" data-payload="${esc}"><span>${num}</span></button>${cleanText}</span>`;
    });
...
// 消息处理
async function appendMessage(sender, message, originalUserInput = null, options = {}) {
    if (sender !== '用户' && !options.skipTraditional) {
...
async function sendMessage() {
    ...
    showLoading();
    try {
        const resp = await fetch((window.API_BASE || '') + '/api/search', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                query: rawUserMessage,
                category: selectedCategory,
                lang: window.userInputWasTraditional ? 'zh-TW' : (window.selectedLang || 'zh-CN'),
            })
        });
        removeLoading();
        ...
        if (data.message) {
            appendMessage('AI', data.message, rawUserMessage, {
                skipTraditional: window.userInputWasTraditional,
            });
        } else {
            const noAnswerText = translations[window.selectedLang]?.noAnswer || "暂时未找到您要的答案，您可以尝试其他问题！";
            appendMessage('AI', noAnswerText);
        }
    } catch (err) {
        removeLoading();
        console.error('Search API error:', err);
        appendMessage('AI', '搜索服务暂时不可用，请稍后再试。');
    }
    scrollToBottom();
...
async function handleLocalDictionaryMatch(userMessage) {
    const originalInput = userMessage.trim();
    const simplifiedInput = convertTraditionalToSimplified(originalInput);
    const rawInput = standardizeQuestion(simplifiedInput);
    ...
    for (const category of categories) {
        ...
        // 【保持原有的所有匹配逻辑不变】
        let exactMatch, containsMatches, fuzzyResults;
        ...
        if (category.name === '经节') {
            ...
            const dataSources = [
                ...
                },
                {
                    ...
                    supportsFuzzy: false,  // 改为false，禁用模糊匹配
                    supportsContains: false, // 改为false，禁用包含匹配
                    render: (k, v) => `${v}`
                }
            ];
...
async function handleChapterFormatQuery(query) {
    ...
    try {
        ...
        // 5. 提取并处理HTML内容
        const processedContent = typeof extractHTMLContent === 'function' ? 
                                extractHTMLContent(htmlContent) : htmlContent;
