...
3. 修正领域专有词，例如：「那灵」「经纶」「召会」「活力排」「职事」「擘饼」「李常受」「倪柝声」「恢复本圣经」「生命读经」「晨兴圣言」「爱有效能」「膏油」「吃基督」「享受基督」「神圣罗曼史」「雷玛」「吗哪」「成为一灵」「生机的救恩」「法理的救赎」「生命之灵的律」「总括时期」「末后的亚当」「高峰真理」「属灵」「作王」「三一神」「赐生命的灵」「祂」「联结」「撒但」「在生命中作王」「承继」「结晶读经」「神人二性」「宇宙合并」「属灵新陈代谢」「终极完成之灵」「加强时期」「总括时期」「得胜者」「职事」「祭司」等
4. 修正以下已知的固定误识对（左边是 Whisper 可能输出的错误形式，右边是正确写法）：
   - 「鼠灵」「鼠龄」→「属灵」
   - 「做完」「作完」→「作王」（仅在「生命中」附近出现时）
   - 「火力牌」「活力牌」→「活力排」
   - 「经轮」「经伦」→「经纶」
   - 「撒旦」→「撒但」
   - 「霓裳」「倪拓」「倪托」→「倪柝」（后接「声」字时）
...
- 不得调整语序
- 如果不确定某个词是否需要修改，原样保留
- 不得改写语气词、副词或虚词，例如不能将「怎么」改为「怎样」
- 不得将识别错误的词强行套用词表中的近似词，例如「次生命的灵」不应改为「赐生命的那灵」
- 不得将识别正确的词改写为词表中的近似词，例如不能将「生机的拯救」改为「生机的救恩」，「拯救」和「救恩」是不同的词
- 遇到阿拉伯数字表示的章节号，将其还原为汉字，例如「15章」→「十五章」、「21章」→「二十一章」；同时去除书名号《》
