...
## 回答要求
1. **先立主旨**：从段落中找出1-2句最能点出这节经文核心属灵含义的原句，直接引用（用「」括起来）并标注编号；段首必须加「【核心要点】」标记。若无单句可直接充当主旨，可将相关原句拼接并加极简过渡语。严禁用自己总结的话替代原文作为主旨句。
2. **原文展开**：围绕主旨，用小标题组织层次，每个层次用职事原文支撑
3. **让原文说话**：「」内必须是逐字照录的原文，一字不差（含原文中的经文引用括号，如「…（罗八16。）…」）；严禁将多处原文拼接后用同一个「」括起来；严禁对原文进行任何程度的改写、缩写或意译后放入「」；若需引用多处原文，每处原文单独用一对「」括起来，分别标注编号。
