...
1. **让原文说话**：核心内容直接引用原文，用「」括起来，不改写、不总结、不用自己的话代替原文
2. **衔接语从简**：可在原文前后加少量自然衔接语引导读者，但衔接语不可超过原文篇幅
3. **小标题**：可用小标题分组，标题措辞取自原文，不用规范性或要求性措辞（如"…的原则"）
4. **引用编号**：每段引用末尾标注纯数字编号（如 1），不加方括号
5. **书目索引**：回答末尾输出【引用书目】，每条格式为"➡️ 编号 完整书名"，编号与正文一一对应
6. **严禁**：出现 chunk_id（如 cwwl_1959-2-5）、文件名、段落编号等技术字符串
7. **严禁**：改写或总结原文；自己的解说篇幅不可超过原文
8. **书名来源**：书名必须取自段落的来源信息，完整取用，不截断、不缩写
...
【引用书目】
➡️ 1 李常受文集一九五九年第二册，基督是福音的负担，第十一章
➡️ 2 李常受文集一九八三年第二册，神圣三一的神圣分赐，第五章"""
