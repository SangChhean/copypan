              ...
              <div v-if="!msg.streaming" class="qa-actions">
                ...
                <div v-if="msg.found" class="qa-action-row2">
                  ...
                  <button
                    ...
                    :title="ttsMsgEngine.get(msg.id) === 'minimax' && ttsState(msg) === 'playing' ? (msg.currentLang === 'en' ? 'Pause' : msg.currentLang === 'zh_tw' ? '暫停' : '暂停') : (msg.currentLang === 'en' ? 'Read' : msg.currentLang === 'zh_tw' ? '朗讀' : '朗读')"
                  >
                    ...
                    <span v-else>{{ msg.currentLang === 'en' ? '🔊 Read' : msg.currentLang === 'zh_tw' ? '🔊 朗讀' : '🔊 朗读' }}</span>
                  </button>
                  <button
...
const usageDisplayText = computed(() => {
  ...
  if (limit === -1) {
    return headerLang.value === 'en' ? `Today ${used}/∞` : `今日 ${used}/不限`
  }
...
async function fetchDailyUsage() {
  ...
  try {
    ...
    const qa = data.qa || {}
    dailyUsage.value = {
      used: Number(qa.used) || 0,
      limit: Number(qa.limit ?? 3),
    }
  } catch {
    /* ignore */
  }
...
function incrementDailyUsageLocal() {
  const { used, limit } = dailyUsage.value
  if (limit === -1) {
    dailyUsage.value = { used: used + 1, limit }
    return
  }
  if (limit <= 0) return
...
function renderAnswer(text) {
  ...
  html = html.replace(/【核心要点】/g, '<span class="qa-key-point">【核心要点】</span>')
  ...
  html = html.replace(/(?<!\d)([.!?。！？])(\d{1,2})(?!\d)/g, (match, punct, num) => {
...
.qa-answer-body :deep(.qa-key-point) {
