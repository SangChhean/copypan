...
EN_TEMPLATE = BACKEND_DIR / "英文刷格式template.docx"
...
@router.post("/api/testa/translate/format_download")
async def format_download(req: FormatRequest):
    ...
    if lang == "zh":
        processed = process_zh_text(req.text)
        ...
        processed = process_en_text(req.text, lang)
        if lang == "es":
            scripture_marker = "Lectura bíblica:"
        else:
            scripture_marker = "Scripture Reading:"
        ...
        for i, line in enumerate(lines_list):
            if line.strip().lower().startswith(scripture_marker.lower()):
                scripture_idx = i
                break
        docx_bytes = make_en_docx(processed, lang)
        filename = generate_filename(lines_list, scripture_idx, lang)

    ascii_name = filename.encode("ascii", "ignore").decode() or f"formatted_{lang}.docx"
    disposition = (
