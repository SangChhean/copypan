...
# ── 索引 ──────────────────────────────────────────────────
INDEX_20 = 'galatians'  # PanAI 2.0 用
INDICES_35 = ','.join([
...
# ── Claude 调用 ───────────────────────────────────────────
async def call_claude(prompt: str, system: str = None, max_tokens: int = 4096) -> str:
    def _sync():
...
async def generate_outline_35(req: Query35Request):
    ...
    if use_preset:
        ...
        logger.info(f'[3.5] 使用预设概念，跳过 Step1')
    else:
        burden_line = f'信息负担说明：{req.burden_description}' if req.burden_description.strip() else ''
