...
# ── Prompt 模板（直接来自 back_mic/backend/ai_search/feast_outline_prompts.py）──
...
class MorningRevivalRequest(BaseModel):
    content: str  # 晨兴信息选读原文

class TranscriptRequest(BaseModel):
    original_outline: str       # 纲目原文
    transcript: str             # 听抄稿
    transcript_preface: Optional[str] = None   # 序言原文（可选）
    transcript_addendum: Optional[str] = None  # 添言原文（可选）

class CompositeRequest(BaseModel):
    morning_revival_outline: str  # 晨兴纲目（已生成）
    transcript_outline: str       # 听抄稿纲目（已生成）


# ── 路由 ──

@router.post("/api/testc/feast_outline/morning_revival")
async def generate_morning_revival(req: MorningRevivalRequest):
    if not req.content.strip():
        raise HTTPException(status_code=422, detail="content 不能为空")
    try:
        prompt = MORNING_REVIVAL_OUTLINE.replace("{content}", req.content.strip())
        outline = await call_claude(prompt)
        return {"outline": outline.strip()}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))


@router.post("/api/testc/feast_outline/transcript")
async def generate_transcript(req: TranscriptRequest):
    if not req.original_outline.strip():
        raise HTTPException(status_code=422, detail="original_outline 不能为空")
    if not req.transcript.strip():
        raise HTTPException(status_code=422, detail="transcript 不能为空")
    try:
        prompt = TRANSCRIPT_OUTLINE.replace(
            "{original_outline}", req.original_outline.strip()
        ).replace("{transcript}", req.transcript.strip())

        tp = (req.transcript_preface or "").strip()
        ta = (req.transcript_addendum or "").strip()

        # 主纲目、序言、添言三路并发
        tasks = [call_claude(prompt)]
        if tp:
            tasks.append(call_claude(PREFACE_OUTLINE.replace("{content}", tp)))
        if ta:
            tasks.append(call_claude(ADDENDUM_OUTLINE.replace("{content}", ta)))

        results = await asyncio.gather(*tasks)

        out = {"outline": results[0].strip()}
        idx = 1
        if tp:
            out["preface_outline"] = results[idx].strip()
            idx += 1
        if ta:
            out["addendum_outline"] = results[idx].strip()
        return out
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))


@router.post("/api/testc/feast_outline/composite")
async def generate_composite(req: CompositeRequest):
    if not req.morning_revival_outline.strip():
        raise HTTPException(status_code=422, detail="morning_revival_outline 不能为空")
    if not req.transcript_outline.strip():
        raise HTTPException(status_code=422, detail="transcript_outline 不能为空")
    try:
        prompt = COMPOSITE_OUTLINE.replace(
            "{transcript_outline}", req.transcript_outline.strip()
        ).replace("{morning_revival_outline}", req.morning_revival_outline.strip())
        outline = await call_claude(prompt)
        return {"outline": outline.strip()}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
