          ...
          <div v-else class="qa-bubble qa-bubble-assistant">
            ...
            <div v-if="!msg.loading" class="qa-disclaimer">
              <span v-if="msg.currentLang === 'en'">
                The above answer is generated by AI based on ministry materials. Please verify against the original text.
              </span>
...
const langShortLabel = { zh: '简', zh_tw: '繁', en: 'EN' }
...
async function submit() {
  ...
  const assistantMsg = {
    ...
    currentLang: 'zh',
    translating: false,
    /** 创建本条 message 时锁定的请求语言（done 后用于决定是否自动切换显示） */
    requestedLang: selectedLang.value,
  }
  ...
  try {
    ...
    const response = await fetch('/api/qa/stream', {
      ...
      body: JSON.stringify({
        ...
        skip_cache: false,
        debug: false,
        lang: assistantMsg.requestedLang,
      ...
      while ((sepIdx = buffer.indexOf('\n\n')) !== -1) {
        ...
        for (const line of rawEvent.split('\n')) {
          ...
          if (chunk.type === 'token') {
            ...
            row.loading = false
            row.streaming = false
          } else if (chunk.type === 'translation') {
            const row = assistantRow()
            if (row && (chunk.lang === 'zh_tw' || chunk.lang === 'en')) {
              row.translatedAnswers[chunk.lang] = {
                answer: chunk.answer || '',
                sources: chunk.sources || [],
              }
              // 用户当前选中的就是该语言（且本条 message 是按该语言提问的）→ 自动切显
              if (selectedLang.value === chunk.lang && row.requestedLang === chunk.lang) {
                row.currentLang = chunk.lang
              }
            }
