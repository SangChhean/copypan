...
_STEP1_MULTI = (
    _STEP1_COMMON
    + """
另需输出 title：标题。由本周全部原文各篇的题目自然结合为一个通顺的标题；只有1篇原文时，直接使用该篇题目本身。
（只写题目本身，不要带「第X周」前缀，也不要带「第X篇」。）

多篇的情况下，"自然结合"的意思是：把各篇标题的核心内容重新组织成一句完整、通顺的话，用"以及"、"与"、"和"这类自然连词衔接，读起来要像是本来就写成一句话的标题，不能是把原文各篇的标题原样保留、中间用破折号"—"、分号"；"这类标点直接焊接起来。

错误示范（这是逐字拼接，不合格）：
创世记—总纲与中心思想；撒但的背叛与败坏

正确示范（这是自然结合，合格）：
创世记的总纲与中心思想以及撒但的背叛与败坏

请特别注意：原文各篇标题内部可能自带破折号（比如"创世记—总纲与中心思想"这种"主题—副标题"的格式，这是生命读经原书单篇标题的常见写法），但合成多篇标题的时候，不需要原样保留这个内部标点结构，应该把内容拆解重组，用连词自然衔接成一句话。
