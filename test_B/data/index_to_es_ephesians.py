# -*- coding: utf-8 -*-
"""创建 ephesians-test 索引并 bulk 写入以弗所书 chunk。"""
import argparse
import json
import os
import time
...
# 与 back_mic/backend/es_config.py 一致：优先加载项目 .env
_BACKEND_ENV = Path(__file__).resolve().parents[2] / "back_mic" / "backend" / ".env"
...
def create_index(es_client: Elasticsearch, index_name: str) -> None:
    """
    创建索引。
    若 ES 未安装 IK 分词插件会抛出异常，脚本层捕获后提示："ES 未安装 IK 分词插件，请先安装 analysis-ik 后重试"。
    """
    try:
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="以弗所书 chunk 索引创建与批量写入 ES"
    )
    parser.add_argument(
        "--input",
        type=str,
    ...
    parser.add_argument(
        "--es-user",
        type=str,
        default=os.getenv("ES_USERNAME", "elastic"),
        help="ES 用户名（默认 elastic 或环境变量 ES_USERNAME）",
    )
    parser.add_argument(
        "--es-password",
        type=str,
        default=os.getenv("ES_PASSWORD", ""),
        help="ES 密码（默认环境变量 ES_PASSWORD）",
    )
    ...
    # ES 8.x 默认开启安全认证
    kwargs["basic_auth"] = (args.es_user or "elastic", args.es_password or "")
