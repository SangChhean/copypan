...
router = APIRouter(prefix="/api/testb/feast_outline")
...
# 序言：将用户输入的序言内容整理成纲目格式（用于听抄稿纲目第四段起）
...
# 添言：将用户输入的添言内容整理成纲目格式（用于听抄稿纲目末尾）
...
async def call_claude(prompt: str, max_tokens: int = 10000) -> str:
...
class TranscriptRequest(BaseModel):
    ...
    transcript: str
    transcript_preface: str = ""
    transcript_addendum: str = ""
...
async def transcript(req: TranscriptRequest):
    if not req.original_outline.strip() or not req.transcript.strip():
        raise HTTPException(status_code=400, detail="original_outline 和 transcript 不能为空")
    ...
    async def _preface():
        if not req.transcript_preface.strip():
            return ""
        prompt = PREFACE_OUTLINE.format(content=req.transcript_preface)
        return await call_claude(prompt)

    async def _addendum():
        if not req.transcript_addendum.strip():
            return ""
        prompt = ADDENDUM_OUTLINE.format(content=req.transcript_addendum)
        return await call_claude(prompt)

    outline, preface_outline, addendum_outline = await asyncio.gather(
        _main(), _preface(), _addendum()
    )
    return {
        "outline": outline,
        "preface_outline": preface_outline,
        "addendum_outline": addendum_outline,
    }


@router.post("/composite")
async def composite(req: CompositeRequest):
    if not req.morning_revival_outline.strip() or not req.transcript_outline.strip():
        raise HTTPException(status_code=400, detail="morning_revival_outline 和 transcript_outline 不能为空")
