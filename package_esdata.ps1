...
if ($StopES) {
    Write-Host "[1/3] 停止 Elasticsearch 8 容器..." -ForegroundColor Yellow
    docker stop elasticsearch8 2>$null
    if ($LASTEXITCODE -eq 0) { Write-Host "  已停止 elasticsearch8" -ForegroundColor Green } else { Write-Host "  未运行或已停止" -ForegroundColor Gray }
...
if ($StopES) {
    Write-Host "[3/3] 重新启动 Elasticsearch 8..." -ForegroundColor Yellow
    docker start elasticsearch8 2>$null
    if ($LASTEXITCODE -eq 0) { Write-Host "  已启动 elasticsearch8" -ForegroundColor Green } else { Write-Host "  启动失败或未安装容器" -ForegroundColor Red }
} else {
