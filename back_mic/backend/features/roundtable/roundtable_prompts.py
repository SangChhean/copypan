...
你的回答必须严格基于题目/材料中提供的具体信息进行分析和判断，不可脱离题目信息自行假设、推断或编造题目未提及的情况。如果题目信息不足以支撑某个具体结论，请明确说明「现有信息不足以判断___」，而不是给出一个看似合理但缺乏依据的答案。
...
【你的任务】
请点名回应至少 2～3 位其他参与者（例如：对 Claude 的观点我认为…；GPT 提到的…值得补充…）。回应时，除了检查对方逻辑是否自洽，还要特别核查对方的观点/建议是否真正基于题目提供的具体信息——如果对方的回答脱离了题目实际情况（比如虚构了题目未提及的细节、忽略了题目里明确提到的约束条件），这属于需要指出的问题，不能因为对方表达流畅、逻辑通顺就放过。要针对他们具体说了什么来回应，不要泛泛而谈。行文使用自然段落，不使用 Markdown 标题符号（#）或列表符号（-、*）。不要提问，直接开始回答。
""" + FORMAT_NO_MARKDOWN
...
【你的任务】
请基于以上讨论，对题目/材料本身给出你的最终评价或结论。给出评价时，请专门检查：前两轮讨论中出现的具体判断或建议，是否都能在题目提供的信息里找到依据。如果某个判断是脱离题目实际情况凭空得出的，请在你的评价中明确标注这一点。行文使用自然段落，不使用 Markdown 标题符号（#）或列表符号（-、*）。不要提问，直接开始回答。
""" + FORMAT_NO_MARKDOWN
...
三、给出对读者有参考价值的结论或建议。在正式写出每一条建议之前，请先在心里自行核查：这条建议是否能在题目提供的信息中找到对应依据？该核查只作内部判断，不要写进正文，也不要出现「经核查」「已核实」「经逐条确认」等说明性文字。只需把核查结果落实在最终写法上：有据可依的，就正常、直接写出，不必额外声明理由多么充分；缺乏依据或题目信息不足以支撑的，不要写成确定性建议，可自然改写为「建议先确认___信息后再决定」，或干脆不写这一条。结论应当保守，宁可少给建议，也不能给出脱离题目实际情况的建议。最终呈现给读者的应是一份干净、直接的结论文本——看不出「我核查过」的痕迹，只看得出结论本身是保守、有依据的。
