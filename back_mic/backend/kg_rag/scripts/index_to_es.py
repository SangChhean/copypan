# -*- coding: utf-8 -*-
"""创建 kg-rag_test 索引并 bulk 写入 chunk。"""
import argparse
import json
import time
...
# 设计文档第五节：KG-RAG 索引 Mapping（embedding 1024 维，写入阶段不填）
INDEX_MAPPING = {
    "mappings": {
        "properties": {
...
# 写入时使用的字段（不含 embedding，由 generate_embeddings.py 回填）
BULK_SOURCE_FIELDS = (
    "chunk_id",
    "text",
    "book_title",
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="KG-RAG 索引创建与批量写入"
    )
    parser.add_argument(
        "--input",
        type=str,
        required=True,
        help="chunking.py 输出的 chunk JSON 文件路径（必填）",
    )
    parser.add_argument(
        "--es-url",
        type=str,
