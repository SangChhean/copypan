# -*- coding: utf-8 -*-
"""将 merged_output3.txt 合并进 seed_concepts_1.json，并输出越界关系清单。"""
...
def main() -> None:
    ...
    merged3 = SCRIPTS / "merged_output3.txt"
    report = ROOT / "seed_concepts_1_merged_output3_out_of_vocab.txt"
    ...
    lines = merged3.read_text(encoding="utf-8").splitlines()
    parsed = []
    ...
    report_lines = [
        f"merged_output3 原始行数: {len(lines)}",
        f"可解析关系行: {len(parsed)}",
    ...
    print(
        ...
        f"out_of_vocab_added={len(out_of_vocab)} bad={len(bad_lines)} total_relations={len(relations)}"
    )
