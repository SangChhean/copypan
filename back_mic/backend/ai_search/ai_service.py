...
_ERROR_CHAR_PAIRS: List[Tuple[str, str]] = [
    ...
    ("鎗", "槍"), ("顋", "腮"), ("效忠", "効忠"), ("抱雛", "菢雛"),
    ("計劃", "計畫"), ("提醒", "題醒"), ("借著", "藉著"),
    ("口喫", "口吃"), ("撒旦", "撒但"), ("形象", "形像"), ("峯", "峰"),
    ("翻譯", "繙譯"), ("對像", "對象"), ("psrp", "禱、研、背、講"),
    ("bnpb", "生、養、教、建"), ("創世紀", "創世記"), ("了結以往", "了結已往"),
    ("景像", "景象"), ("掛慮", "罣慮"), ("束板", "豎板"), ("先軀", "先驅"),
    ("深遂", "深邃"), ("喜悦", "喜悅"), ("才知道", "纔知道"),
    ("複習", "復習"), ("重覆", "重複"), ("撲倒", "仆倒"),
    ("必像必肖", "畢像畢肖"), ("罪於死的律", "罪與死的律"), ("發芽結石", "發芽結實"),
]
_ERROR_PATTERNS_SORTED: List[Tuple[str, str]] = sorted(
    _ERROR_CHAR_PAIRS, key=lambda p: len(p[0]), reverse=True
)
_ERROR_CHAR_SINGLES: List[str] = [
    "每周", "瞭", "提", "注", "謀劃", "凉", "凟", "况", "冲", "凄",
    "盗", "凖", "略", "啟", "裡", "猫", "鷄", "摻", "綫", "吃", "夠", "却",
    "脚", "寳", "唯", "餵", "嗎", "吧", "証", "台", "份", "嘆", "擧", "耻",
    "葯", "腊", "覇", "净", "濶", "恒", "隣", "做", "烟", "床", "踪", "仿",
    "祢", "唇", "禾秸", "台", "哪裡", "羡", "效力", "磚", "拋磚引玉", "吃",
    "牠", "軟", "教會", "啊", "凟", "燄", "揑", "撑", "饍",
    "榥", "愓", "覇", "隣", "略", "群", "戞", "裡", "猫", "鷄", "㕑", "猪",
    "擧", "冤", "妬", "枱", "溼", "摻", "濶", "恒", "灯", "預", "綫",
    "吃", "夠", "却", "脚", "寳", "秘", "做", "烟", "嘆", "唸", "台", "嚐",
    "鬥", "厦", "纒", "橱", "磚", "嘠", "犁", "煖", "唯", "軟", "餵", "什",
    "嗎", "抺", "吧", "証", "踪", "仿", "耽", "鎗", "份", "耻", "刄", "朶",
    "搯", "强", "滙", "囘", "塲", "䧟", "葯", "腊", "逈", "粵", "啟", "罵",
    "妳", "祢", "唇", "牠", "衛", "麪", "跡", "敘", "閑", "鉤", "刧", "刼",
    "床", "僣", "啣", "遍", "并", "借", "詞", "着作", "程式", "麻風", "于皮",
    "诗歌", "预", "塞缪尔", "雅各布", "艾萨克", "乔书亚", "乔伯", "伊莱贾",
    "線上", "迴圈", "回復", "閑", "資訊", "周", "麽", "週", "異像", "曆世",
    "併", "才",
]
# 去重并保持顺序
_ERROR_CHAR_SINGLES = list(dict.fromkeys(_ERROR_CHAR_SINGLES))
...
def scan_error_chars(content: str) -> List[Dict[str, Optional[str]]]:
    ...
    while i < n:
        ...
        if term:
            ...
            continue
        i += 1
    # 第三层：_ERROR_CHAR_SINGLES 扫描（无建议词，suggestion = word 本身）
    singles_sorted = sorted(_ERROR_CHAR_SINGLES, key=len, reverse=True)
    j = 0
    while j < n:
        matched = False
        for word in singles_sorted:
            if word and content.startswith(word, j):
                # 避免与已有 hits 重叠
                if not any(h["start"] <= j < h["end"] for h in hits):
                    hits.append({
                        "word": word,
                        "start": j,
                        "end": j + len(word),
...
class AISearchService:
    ...
    def outline_to_traditional(self, content: str) -> Dict[str, Optional[str]]:
        ...
        try:
            ...
            # 4. 静默替换易错字（_ERROR_CHAR_PAIRS，无需用户确认）
            for wrong, right in _ERROR_CHAR_PAIRS:
