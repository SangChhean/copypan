...
7、纲目的序号与纲目中间必须用Tab键连接，不可用顿号、全角空格、逗号或其他的符号链接。
8、若一条纲目有下一级纲目，则这一条纲目句尾需加冒号，若这一条纲目没有下一级纲目，则这一条纲目句尾经节出处后面需加句号，题目后面不需要加经节。
9、请记住纲目中的所有单引号必须用中文状态下的单引号，不可用英文状态下的单引号。
10、一条纲目句中若有句号，需将句号改成分号。
11、纲目不可以重复。
12、纲目篇幅必须严格在A4纸三页之内，不得超过，不得超过。若篇幅超过三页，应优先精简中纲及小纲内容，但罗马大纲结构不可删减。
...
10、提取的内容不应作为独立的大纲级别（壹、贰、叁等）添加到复合纲目中，而应作为现有大纲级别下的子级内容（如一、二、三或1、2、3或a、b、c等）进行添加；添加的内容应自然地融入原纲目的现有结构中，而不是创建新的大纲层级，确保所有添加的内容都能与上下文逻辑连贯，并且不破坏原有的纲目结构和层次关系。
11、从听抄稿提取的内容，若每个纲目有下一级的纲目，下一级纲目至少需要提取2个。
12、原纲目中每个原有纲目点的层级结构（壹、贰、叁或一、二、三或1、2、3或a、b、c）绝对不能改变，只能调整序号以保持连续性，整合后确保纲目序号连续且没有重复的纲目序号出现。
13、整合后的纲目应保持统一的格式，纲目序号与纲目内容之间必须用Tab键连接，不可用顿号、全角空格或其他符号连接。
14、一条纲目句中若有句号，需将句号改成分号。
15、在最终文档中，清晰标记从听抄稿添加的内容，标记形式为：换行符号＋【添加开始】＋换行符号＋添加的内容＋换行符号＋【添加结束】＋换行符号。
16、用代码块格式回复。
...
3、确保添加内容与听抄稿纲目内容内涵不重复，不会高度相似（高度相似：相似度超过80％）。
...
def get_preface_outline_prompt(content: str) -> str:
    return PREFACE_OUTLINE.replace("{content}", content)


def get_addendum_outline_prompt(content: str) -> str:
    return ADDENDUM_OUTLINE.replace("{content}", content)
