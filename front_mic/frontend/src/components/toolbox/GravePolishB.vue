...
const article = ref("");
const roles = ref({}); // { key: label }
const selectedRoles = ref([]); // [key, ...]
...
const canSubmit = computed(
  () => article.value.trim().length > 0 && selectedRoles.value.length > 0
);

function toggleRole(key) {
  const idx = selectedRoles.value.indexOf(key);
  if (idx === -1) {
...
function isSelected(key) {
  return selectedRoles.value.includes(key);
}
...
async function loadRoles() {
  try {
    const res = await fetch("/api/testb/grave-roles", {
      method: "POST",
...
async function generate() {
  ...
  results.value = [];
  try {
    const res = await fetch("/api/testb/grave-polish", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
    ...
    results.value = selectedRoles.value
      .filter((key) => key in resultMap)
      .map((key) => ({
        key,
        label: roles.value[key] || key,
        text: resultMap[key],
      }));
  } catch (err) {
...
<template>
  <ToolsHeader title="润色恩典陵园" />
  <div class="box">
    <a-card>
      <p class="hint">
        输入见证稿内容，选择一个或多个润色角色，点击「开始润色」，每个角色的润色结果会单独展示。
      </p>
      <a-divider :style="{ margin: '12px 0' }" />

      <div class="style-section">
        <span class="label">选择润色角色（可多选）：</span>
        ...
        <div class="role-desc">
          <p>同工角色适合润色于：真理性强或要求精炼有力度的见证稿</p>
          <p>亲友角色适合润色于：要求要有深入富有情感表达的见证稿</p>
          <p>编辑者角色适合润色于：专业严谨通用性强的见证稿，如倪弟兄、李弟兄的见证</p>
        </div>
      </div>
      ...
      <p v-if="loading" class="loading-hint">请耐心等待，多个角色会一起处理</p>
    </a-card>
