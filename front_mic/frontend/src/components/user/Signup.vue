...
import { UserOutlined, LockOutlined, SendOutlined } from "@ant-design/icons-vue";
...
const onFinish = (values: any) => {
  ...
  formData.append("ivcode", values.ivcode);
  axios
...
const disabled = computed(() => {
  ...
  let ivcode = formState.ivcode.trim();
  if (username.length < 3 || username.length > 16 || /^[a-zA-Z0-9_]+$/.test(username) == false) {
    return true;
  } else if (password.length < 6 || password.length > 16 || /^[a-zA-Z0-9_]+$/.test(password) == false) {
    return true;
  } else if (!ivcode) {
    return true;
  }
    ...
    <div id="form">
      <a-card :hoverable="true">
        ...
        <a-form :model="formState" name="normal_login" class="login-form" @finish="onFinish" @finishFailed="onFinishFailed">
          ...
          <a-form-item label="邀请码" name="ivcode" :rules="[{ required: true, message: '请输入您的邀请码!' }, { whitespace: true }]">
            <a-input v-model:value="formState.ivcode" allowClear>
              <template #prefix>
          ...
          <a-form-item>
            <a-button :disabled="disabled" type="primary" html-type="submit" class="login-form-button"> 注册 </a-button>
            <span>已有账号？</span>
