  ...
  axios
    ...
    .then((res) => {
      if (res.data.status === "success") {
        stats.value = res.data.data?.toolbox?.translation || {};
      } else {
...
const tableColumns = [
  { title: "翻译方向", dataIndex: "name", width: 120 },
...
const tableRows = computed(() => {
  const tr = stats.value || {};
  const zh2en = tr.zh2en || { count: 0, cost: 0 };
  const en2zh = tr.en2zh || { count: 0, cost: 0 };
  const totalCount = zh2en.count + en2zh.count;
  const totalCost = zh2en.cost + en2zh.cost;
  return [
    { key: "zh2en", name: "中→英", count: zh2en.count, cost: zh2en.cost },
    { key: "en2zh", name: "英→中", count: en2zh.count, cost: en2zh.cost },
